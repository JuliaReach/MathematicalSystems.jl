@testset "vector_field method and `VectorField` for continuous systems" begin
    ...
    sys = NoisyConstrainedBlackBoxControlContinuousSystem(f, size(A, 2), size(B, 2), size(D, 2), X,
                                                          U, W)
...
@testset "vector_field exception handling" begin
    ...
    sys = ContinuousIdentitySystem(size(A, 2))
    @test_throws ArgumentError("the state vector has the wrong dimensions") vector_field(sys,
                                                                                         x_wrong_dim)
    sys = ConstrainedContinuousIdentitySystem(size(A, 2), X)
    @test_throws ArgumentError("the state vector is not contained in the state set") vector_field(sys,
                                                                                                  x_not_in_set)
    sys = LinearContinuousSystem(A)
    @test_throws ArgumentError("the state vector has the wrong dimensions") vector_field(sys,
                                                                                         x_wrong_dim)
    ...
    @test_throws ArgumentError("the state vector has the wrong dimensions") vector_field(sys,
                                                                                         x_wrong_dim)
    sys = LinearControlContinuousSystem(A, B)
    @test_throws ArgumentError("the input vector has the wrong dimensions") vector_field(sys, x,
                                                                                         u_wrong_dim)
    sys = ConstrainedLinearContinuousSystem(A, X)
    @test_throws ArgumentError("the state vector is not contained in the state set") vector_field(sys,
                                                                                                  x_not_in_set)
    ...
    @test_throws ArgumentError("the state vector is not contained in the state set") vector_field(sys,
                                                                                                  x_not_in_set)
    sys = ConstrainedLinearControlContinuousSystem(A, B, X, U)
    @test_throws ArgumentError("the input vector is not contained in the input set") vector_field(sys,
                                                                                                  x,
                                                                                                  u_not_in_set)
    sys = ConstrainedAffineControlContinuousSystem(A, B, c, X, U)
    @test_throws ArgumentError("the input vector has the wrong dimensions") vector_field(sys, x,
    ...
    @test_throws ArgumentError("the input vector is not contained in the input set") vector_field(sys,
                                                                                                  ...
                                                                                                  u_not_in_set)
    sys = NoisyConstrainedLinearContinuousSystem(A, D, X, W)
    @test_throws ArgumentError("the noise vector is not contained in the noise set") vector_field(sys,
                                                                                                  x,
                                                                                                  w_not_in_set)
    sys = NoisyConstrainedLinearControlContinuousSystem(A, B, D, X, U, W)
