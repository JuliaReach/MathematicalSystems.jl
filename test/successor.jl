@testset "successor method for discrete systems" begin
    ...
    sys = NoisyConstrainedBlackBoxControlDiscreteSystem(f, size(A, 2), size(B, 2), size(D, 2), X, U,
                                                        W)
...
@testset "successor exception handling" begin
    ...
    sys = DiscreteIdentitySystem(size(A, 2))
    @test_throws ArgumentError("the state vector has the wrong dimensions") successor(sys,
                                                                                      x_wrong_dim)
    sys = ConstrainedDiscreteIdentitySystem(size(A, 2), X)
    @test_throws ArgumentError("the state vector is not contained in the state set") successor(sys,
                                                                                               x_not_in_set)
    sys = LinearDiscreteSystem(A)
    @test_throws ArgumentError("the state vector has the wrong dimensions") successor(sys,
                                                                                      x_wrong_dim)
    ...
    @test_throws ArgumentError("the state vector has the wrong dimensions") successor(sys,
                                                                                      x_wrong_dim)
    sys = LinearControlDiscreteSystem(A, B)
    @test_throws ArgumentError("the input vector has the wrong dimensions") successor(sys, x,
                                                                                      u_wrong_dim)
    sys = ConstrainedLinearDiscreteSystem(A, X)
    @test_throws ArgumentError("the state vector is not contained in the state set") successor(sys,
                                                                                               x_not_in_set)
    ...
    @test_throws ArgumentError("the state vector is not contained in the state set") successor(sys,
                                                                                               x_not_in_set)
    sys = ConstrainedLinearControlDiscreteSystem(A, B, X, U)
    @test_throws ArgumentError("the input vector is not contained in the input set") successor(sys,
                                                                                               x,
                                                                                               u_not_in_set)
    sys = ConstrainedAffineControlDiscreteSystem(A, B, c, X, U)
    @test_throws ArgumentError("the input vector has the wrong dimensions") successor(sys, x,
    ...
    @test_throws ArgumentError("the input vector is not contained in the input set") successor(sys,
                                                                                               ...
                                                                                               u_not_in_set)
    sys = NoisyConstrainedLinearDiscreteSystem(A, D, X, W)
    @test_throws ArgumentError("the noise vector is not contained in the noise set") successor(sys,
                                                                                               x,
                                                                                               w_not_in_set)
    sys = NoisyConstrainedLinearControlDiscreteSystem(A, B, D, X, U, W)
    @test_throws ArgumentError("the noise vector has the wrong dimensions") successor(sys, x, u,
                                                                                      w_wrong_dim)
    sys = NoisyConstrainedAffineControlDiscreteSystem(A, B, c, D, X, U, W)
    @test_throws ArgumentError("the noise vector is not contained in the noise set") successor(sys,
                                                                                               x, u,
